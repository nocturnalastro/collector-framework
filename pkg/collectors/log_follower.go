...
func NewLogsCollector(constructor *CollectionConstructor, ctx *clients.ContainerExecContext) (Collector, error) {
	...
	if !ok {
		return &LogsCollector{}, errors.New("no Logs args in collector args")
	}
	logsOutputFileRaw, ok := logArgs["logsOutputFile"]
	if !ok {
		return &LogsCollector{}, errors.New("no logsOutputFile in Log collector args")
	}

	logsOutputFile, ok := logsOutputFileRaw.(string)
	if !ok {
		return &LogsCollector{}, errors.New("logsOutputFile is not a string")
	}
	...
	if !ok {
		return &LogsCollector{}, errors.New("logsOutputFile is not a bool")
	}

	tempDirRaw, ok := logArgs["tempDir"]
	if !ok {
		return &LogsCollector{}, errors.New("no tempDir in Log collector args")
	}

	tempDir, ok := tempDirRaw.(string)
	if !ok {
		return &LogsCollector{}, errors.New("tempDir is not a string")
	}

	keepDebugFilesRaw, ok := logArgs["keepDebugFiles"]
	if !ok {
		return &LogsCollector{}, errors.New("no keepDebugFiles in Log collector args")
	}

	keepDebugFiles, ok := keepDebugFilesRaw.(bool)
	if !ok {
		return &LogsCollector{}, errors.New("keepDebugFiles is not a string")
	}
