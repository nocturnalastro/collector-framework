...
func NewDevInfoCollector(constructor *CollectionConstructor) (Collector, error) {
	...
	if !ok {
		return &DevInfoCollector{}, errors.New("no PTP args in collector args")
	}
	ptpInterfaceRaw, ok := ptpArgs["PtpInterface"]
	if !ok {
		return &DevInfoCollector{}, errors.New("no PtpInterface in PTP collector args")
	}

	ptpInterface, ok := ptpInterfaceRaw.(string)
	if !ok {
		return &DevInfoCollector{}, errors.New("PTP interface is not a string")
	}
